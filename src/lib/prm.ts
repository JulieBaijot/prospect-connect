...
export type HeadcountRange = "10-19" | "20-49" | "50-99" | "100-199" | "200-249" | "250-499" | "500-999" | "1000+";
export type Category = "A – Pilier" | "B – Socle SST" | "C – Porte d'entrée" | "Récurrent" | "Exceptionnel";
...
export const headcountRanges: HeadcountRange[] = ["10-19", "20-49", "50-99", "100-199", "200-249", "250-499", "500-999", "1000+"];
export const categories: Category[] = ["A – Pilier", "B – Socle SST", "C – Porte d'entrée", "Récurrent", "Exceptionnel"];
export const offerTargets: OfferTarget[] = ["SST FI", "SST MAC", "CSE-CSSCT", "QVCT", "Émotions", "Excel"];
...
export async function loadProspects(): Promise<ProspectWithRelations[]> {
  ...
  return ((data || []) as unknown as ProspectWithRelations[]).map((prospect) => ({
    ...
    prospection_logs: (prospect.prospection_logs || []).sort((a, b) => +new Date(b.action_date) - +new Date(a.action_date)),
  }));
}

export async function loadLogs(): Promise<Array<ProspectionLog & { prospects: Prospect | null; contacts: Contact | null }>> {
  const { data, error } = await supabase
    .from("prospection_logs")
    .select("*, prospects(*), contacts(*)")
    .order("action_date", { ascending: false });
  if (error) throw error;
  return (data || []) as unknown as Array<ProspectionLog & { prospects: Prospect | null; contacts: Contact | null }>;
}

export async function saveProspect(input: Partial<Prospect> & { company_name: string }, contact?: Partial<Contact>) {
  const { data: prospect, error } = await supabase.from("prospects").upsert(input).select().single();
  if (error) throw error;
  if (contact && (contact.first_name || contact.last_name || contact.email || contact.direct_phone || contact.role_title)) {
    const payload = { ...contact, prospect_id: prospect.id };
    const { error: contactError } = await supabase.from("contacts").upsert(payload).select().single();
    if (contactError) throw contactError;
  }
  return prospect as unknown as Prospect;
}

export async function addLog(payload: Partial<ProspectionLog> & { prospect_id: string; action_type: string }) {
...
export function prioritizeSession(prospects: ProspectWithRelations[]) {
  const categoryRank: Record<string, number> = { "A – Pilier": 0, "B – Socle SST": 1, "C – Porte d'entrée": 2, Récurrent: 3, Exceptionnel: 4 };
  const statusRank: Record<string, number> = { Chaud: 0, Tiède: 1, "En attente": 2, Converti: 3, Perdu: 4 };
  return [...prospects]
    ...
    .sort((a, b) => {
      ...
      const byDate = +(new Date(a.next_action_date || "2099-12-31")) - +(new Date(b.next_action_date || "2099-12-31"));
...
export function buildCalendarUrl(args: { prospect: Prospect; contact?: Contact; date: string; duration: number; videoLink?: string }) {
  ...
  const details = [`Contact : ${contactName(args.contact)}`, `Téléphone : ${args.contact?.direct_phone || args.contact?.main_phone || args.prospect.main_phone || "à compléter"}`, `Offre : ${args.prospect.offer_target || "à préciser"}`, args.videoLink ? `Visio : ${args.videoLink}` : ""].filter(Boolean).join("\n");
  const params = new URLSearchParams({ action: "TEMPLATE", text: title, dates: `${toGoogleDate(start)}/${toGoogleDate(end)}`, details, location: args.videoLink || args.prospect.address || "" });
